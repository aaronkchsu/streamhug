...
import InnerAudioButton from "./AudioButton";

export { ButtonModes } from "./AudioButton";
...
const PLAY_BITE_TWITCH = gql`
  mutation twitchIncrementPlayCountOnServer(
    $id: MongoID!
    $featuredPage: String
  ) {
    twitch {
      bitePlay(
        _id: $id
        analytics: { data: { featured: $featuredPage, action: "PLAY" } }
      ) {
        _id
      }
    }
...
const SET_GLOBAL_SRC = gql`
  mutation twitchSetGlobalSrc($id: String!) {
    ...
    return (
      <Container className={this.props.className}>
        <Query query={GET_GLOBAL_ITEMS}>
          {({ loading, error, data }) => {
            return (
              <Mutation mutation={PLAY_BITE_TWITCH}>
                {twitchIncrementPlayCountOnServer => (
                  <Mutation mutation={SET_GLOBAL_SRC}>
                    {setGlobalSrc => (
                      <InnerAudioButton
                        ref={this.innerRef}
                        ...
                        durationChangeCallback={
                          this.props.durationChangeCallback
                        }
                        timeUpdateCallback={this.props.timeUpdateCallback}
                        stopCallBack={this.props.stopCallBack}
                        ...
                        globalSrcId={data.globalSrcId}
                        useGlobalAudio={this.props.useGlobalAudio}
                        size={this.props.size}
                        doNotLogPlay={this.props.doNotLogPlay}
                      />
                    )}
                  </Mutation>
                )}
